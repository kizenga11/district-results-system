...
function render_header(string $title): void
{
    ...
    $toast_titles = ['success' => 'Success', 'error' => 'Error', 'warning' => 'Warning', 'info' => 'Information'];

    $flash_html = '';
    $has_flash  = array_filter($flash_toasts);
    if ($has_flash) {
        $flash_html .= '<div class="toast-container" id="toast-container">';
        foreach ($flash_toasts as $type => $msg) {
            if (!$msg) continue;
            $dur = ($type === 'error') ? '6000' : '4500';
            $flash_html .= '<div class="toast-item toast-' . $type . '" data-dur="' . $dur . '" style="--toast-dur:' . $dur . 'ms">';
            $flash_html .= '<div class="toast-icon">' . $toast_icons[$type] . '</div>';
            $flash_html .= '<div class="toast-body">';
            $flash_html .= '<div class="toast-title">' . $toast_titles[$type] . '</div>';
            $flash_html .= '<div class="toast-msg">' . e($msg) . '</div>';
            $flash_html .= '</div>';
            $flash_html .= '<button class="toast-close" aria-label="Close">&times;</button>';
            $flash_html .= '<div class="toast-progress"></div>';
            $flash_html .= '</div>';
        }
        $flash_html .= '</div>';
    }
...
function render_footer(): void
{
    global $flash_html;
    echo '</div></div>';
    if (!empty($flash_html)) echo $flash_html;
